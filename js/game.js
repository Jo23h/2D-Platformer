...
function animate(){
    ...
    } else if (keys.left.pressed === true && (player.position.x > 100 || distanceTravel <= 0)){
        player.velocity.x = -player.speed
    ...
    } if (keys.left.pressed === true && distanceTravel > 0){
        distanceTravel -= player.speed
        platforms.forEach(platform => {
            platform.position.x += player.speed
        })
...
Promise.all([
    ...
    new Promise(resolve => platformSmall.onload = resolve),
    new Promise(resolve => platformMed.onload = resolve),
    new Promise(resolve => floor.onload = resolve),
    new Promise(resolve => goal.onload = resolve)
]).then(() => {
    // Create platforms after images are loaded
    platforms = [
    ...
    // Create background after image is loaded
    ...
    player = new Player({x:50, y:50, image: playerImg})
